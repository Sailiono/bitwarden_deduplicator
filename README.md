...
- 安全处理附加字段：整行重复直接删除，空字段可补全，真实冲突会保留并报告
- 输出文件默认设置为仅当前用户可读写（`0600`，平台支持时）
- 提供命令行界面，简单易用
...
- 五个登录字段相同只表示“候选重复”。如果备注、TOTP、自定义字段、文件夹等存在不同的非空值，工具会保留这些版本并报告冲突数量，避免静默丢失数据
- 如果候选重复记录的附加字段仅存在“空值/非空值”差异，工具会保留非空值并安全合并
- 域名级别去重功能对于同一网站不同页面的记录特别有用，例如将`https://example.com/`和`https://example.com/login`视为相同网站
- 域名模式是显式的宽松匹配，会保留匹配组中的第一个完整URL，但仍会保护其他附加字段冲突
